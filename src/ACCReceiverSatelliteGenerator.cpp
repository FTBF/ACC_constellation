...
#include "ACCReceiverSatellite.hpp" // <-- Replace with satellite header here
...
std::shared_ptr<constellation::satellite::Satellite> generator(std::string_view type, std::string_view name) {
    return std::make_shared<ACCReceiverSatellite>(type, name); // <-- Replace with satellite class here
}
}
