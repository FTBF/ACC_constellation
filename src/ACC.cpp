...
std::string ACC::versionCheck(bool debug)
{
    ...
    {std::stringstream ss;
        ...
        char buf[216];
		sprintf(buf,"  Header/footer: %4lx %4lx %4lx %4lx (%s)\n", buf[0], buf[1], buf[30], buf[31], (buf[0] == 0x1234 && buf[1] == 0xbbbb && buf[30] == 0xbbbb && buf[31] == 0x4321)?"Correct":"Wrong");
		version += buf;
        sprintf(buf,"  PLL lock status:\n    ACC PLL:    %d\n    Serial PLL: %d\n    JC PLL:     %d\n    SYS PLL:    %d\n    WR PLL:     %d\n", (buf[6] & 0x4)?1:0, (buf[6] & 0x2)?1:0, (buf[6] & 0x200)?1:0, (buf[6] & 0x8)?1:0, (buf[6] & 0x1)?1:0);
        version += buf;
        sprintf(buf,"  FLL Locks:              %8lx\n", (buf[6] >> 4)&0x1f);
        version += buf;
        sprintf(buf,"  ACC 40 MHz:             %8.3f MHz\n", float(buf[7])/1000.0);
        version += buf;
        sprintf(buf,"  JCPLL 40 MHz:           %8.3f MHz\n", float(buf[8])/1000.0);
        version += buf;
		sprintf(buf,"  Backpressure:           %8d\n", (buf[5] & 0x2)?1:0);
        version += buf;
		sprintf(buf,"  40 MBPS parity error:   %8d\n", (buf[5] & 0x1)?1:0);
        version += buf;
		sprintf(buf,"  Event count:            %8lu\n", (buf[15] << 16) | buf[16]);
        version += buf;
		sprintf(buf,"  ID Frame count:         %8lu\n", (buf[17] << 16) | buf[18]);
        version += buf;
		sprintf(buf,"  Trigger count all:      %8lu\n", (buf[11] << 16) | buf[12]);
        version += buf;
		sprintf(buf,"  Trigger count accepted: %8lu\n", (buf[13] << 16) | buf[14]);
        version += buf;
		sprintf(buf,"  PSEC0 FIFO Occ:         %8lu\n", buf[21]);
        version += buf;
		sprintf(buf,"  PSEC1 FIFO Occ:         %8lu\n", buf[22]);
        version += buf;
		sprintf(buf,"  PSEC2 FIFO Occ:         %8lu\n", buf[23]);
        version += buf;
		sprintf(buf,"  PSEC3 FIFO Occ:         %8lu\n", buf[24]);
        version += buf;
		sprintf(buf,"  PSEC4 FIFO Occ:         %8lu\n", buf[25]);
        version += buf;
		sprintf(buf,"  Wr time FIFO Occ:       %8lu\n", buf[26]);
		version += buf;
		sprintf(buf,"  Sys time FIFO Occ:      %8lu\n", buf[27]);
		version += buf;
		sprintf(buf,"\n");
		version += buf;
